  ...
  const renderLink = (
    ...
    if (isHomePage) {
      return (
        <ScrollLink
          ...
          offset={-80}
          duration={500}
          activeClass="nav-active !font-bold"
          className={`${baseClasses} text-slate-300 hover:text-white`}
          onClick={isMobile ? () => setIsMenuOpen(false) : undefined}
        >
          {item.label}
          {!isMobile && (
            <span className="absolute -bottom-1 left-0 w-0 h-0.5 bg-gradient-to-r from-[#61DAFB] to-[#20232A] transition-all duration-300 group-hover:w-full [.nav-active_&]:w-full" />
          )}
        </ScrollLink>
      );
    }
    ...
    return (
      ...
      >
        {item.label}
        {!isMobile && (
          <span className="absolute -bottom-1 left-0 w-0 h-0.5 bg-gradient-to-r from-[#61DAFB] to-[#20232A] transition-all duration-300 group-hover:w-full" />
        )}
      </Link>
    );
  };
